...
public class TransacaoService {
    ...
    private static final String STATUS_UTILIZADO =
            "UTILIZADO";
    ...
    public ResgateResponseDTO resgatar(
        ...
        if (
                vantagem.getEstoque() != null
                        &&
                        vantagem.getEstoque() <= 0
        ) {

            throw new IllegalStateException(
                    "Vantagem sem estoque"
            );
        }

        // atualizar saldo

        aluno.setSaldoMoedas(
                aluno.getSaldoMoedas()
                        - vantagem.getCustoMoedas()
        );
        ...
        ResgateResponseDTO response =
                new ResgateResponseDTO();

        response.setCodigoCupom(
                codigoCupom
        );

        response.setVantagemNome(
                vantagem.getNome()
        );

        response.setCustoMoedas(
                vantagem.getCustoMoedas()
        );

        response.setSaldoRestante(
                aluno.getSaldoMoedas()
        );

        response.setDataResgate(
                salva.getData()
        );
    ...
    public List<TransacaoResponseDTO>
    extratoAluno(
            Long alunoId
    ) {

        if (
                !alunoRepository.existsById(
                        alunoId
                )
        ) {
    ...
    public CupomValidacaoDTO utilizarCupom(
        ...
        if (
            ...
            throw new IllegalStateException(
                    "Cupom não está pendente (status atual: "
                            + t.getStatus()
                            + ")"
            );
        }
    ...
    private void assertCupomDaEmpresa(
            Transacao t,
            Long empresaIdAutenticada
    ) {
        ...
        if (
            ...
            throw new SecurityException(
                    "Cupom não pertence à sua empresa"
            );
        }
    }

    public List<CupomValidacaoDTO>
    cuponsDaEmpresa(
            Long empresaId
    ) {

        return transacaoRepository
                .findCuponsByEmpresaId(
                        empresaId
                )
                .stream()
                .map(this::toCupomDTO)
    ...
    private TransacaoResponseDTO toResponseDTO(
            Transacao t
    ) {

        TransacaoResponseDTO dto =
    ...
    private CupomValidacaoDTO toCupomDTO(
            Transacao t
    ) {

        CupomValidacaoDTO dto =
                new CupomValidacaoDTO();
    ...
    private String extrairNomeVantagem(
            String mensagem
    ) {
    ...
    private String nomeEmpresaOuInstituicao(
            Vantagem vantagem
    ) {
        ...
        if (vantagem.getInstituicao() != null) {
    ...
    private String emailEmpresaOuFallback(
            Vantagem vantagem
    ) {

        if (
                vantagem.getEmpresa() != null
                        &&
                        vantagem.getEmpresa().getEmail() != null
        ) {

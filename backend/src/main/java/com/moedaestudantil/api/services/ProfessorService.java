...
public class ProfessorService {
    ...
    public ProfessorLoginResponseDTO login(
            ProfessorLoginDTO dto
    ) {
        ...
        String senhaCodificada =
                Base64.getEncoder()
                        .encodeToString(
                                dto.getSenha().getBytes()
                        );

        if (!professor.getSenha()
                .equals(senhaCodificada)) {
        ...
        return new ProfessorLoginResponseDTO(
                token,
    ...
    public EnvioMoedasResponseDTO enviarMoedas(
            Long professorId,
            EnviarMoedasDTO dto
    ) {
        ...
        if (dto.getMensagem() == null
            ...
            throw new RuntimeException(
                    "Mensagem de reconhecimento é obrigatória"
            );
        }

        // atualizar saldos

        professor.setSaldoMoedas(
                professor.getSaldoMoedas()
                        - dto.getValor()
        );
        ...
        EmailRecebimentoMoedaEvent recebimentoEvent =
                new EmailRecebimentoMoedaEvent(
        ...
        emailProducerService.publicarRecebimento(
                recebimentoEvent
        );

        EmailEnvioMoedaEvent envioEvent =
                new EmailEnvioMoedaEvent(
        ...
        emailProducerService.publicarEnvio(
                envioEvent
        );
    ...
    public List<TransacaoResponseDTO> extrato(
            Long professorId
    ) {
    ...
    private TransacaoResponseDTO toTransacaoDTO(
            Transacao t
    ) {
    ...
    public List<AlunoResponseDTO>
    listarAlunosPorInstituicao(
            Long professorId
    ) {
    ...
    buscarAlunosPorNome(
        ...
        return alunoRepository
                .findByInstituicaoIdAndNomeContainingIgnoreCase(
                        professor.getInstituicao().getId(),
                        nome
                )
                .stream()
    ...
    private AlunoResponseDTO toAlunoDTO(
            Aluno a
    ) {
